...
class Track(MediaRoot):
  def __init__(self, artist, album, title=None, index=None, year=None, disc=None, album_artist=None, guid=None, filename=None):
  ...
  def __repr__(self):
    return "%s (album: %s, track: %s, guid: %s, filename: %s) #%d => %s" % (self.artist, self.album, self.name, self.guid, self.filename, self.index, self.title)
